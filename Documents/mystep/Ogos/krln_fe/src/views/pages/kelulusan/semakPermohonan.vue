...
onMounted(() => {
    // Log the Permohonan ID when the component is mounted
    fetchMohonID();
    getKategoriProgram();
    getKategoriPeserta();
    getKategoriKhidmat();
...
const selectedPeringkatKelulusan = ref(null); // Direct binding for the dropdown
const isPeringkatRequiredError = ref(false);
...
const filteredUsers = computed(() => users.value.filter((user) => user.Nama.toLowerCase().includes(searchQuery.value.toLowerCase())));
...
const dropdownStatus = ref([]); // Store status options
const statusMapping = {}; // Dynamic mapping for quick lookup
const userMap = ref({});
...
// Fetch status options and build a mapping
const getStatus = async () => {
    try {
        const { data } = await axios.get(`${baseUrl}/api/senaraiStatusMohon`);
        dropdownStatus.value = data.map((item) => ({
            value: item.StatusMohonID,
            label: item.Keterangan.trim()
        }));

        // Build a quick lookup map
        data.forEach((item) => {
            statusMapping[item.StatusMohonID] = item.Keterangan.trim();
        });
    } catch (err) {
        console.error('Failed to load status options:', err.response?.data || err.message);
    }
};

// Fetch kronologi data
const kronologi = async (MohonID) => {
    try {
        const response = await axios.post(`${baseUrl}/api/permohonan/getById`, {
            MohonID: MohonID
        });
        ...
        if (response.data && response.data.length > 0) {
            tableData.value = response.data.map((item, index) => ({
                Bil: index + 1,
                Status: 'Baharu', //statusMapping[item.StatusMohonID] ? statusMapping[item.StatusMohonID] : 'Tidak Diketahui', // Ensure statusMapping is checked before using
                TkhHantar: new Date(item.TkhHantar).toLocaleDateString('en-GB'), // Format date (e.g., DD/MM/YYYY)
                NamaPegawai: userMap.value[item.PenggunaID] || item.PenggunaID, // Use Nama if available, otherwise fallback to PenggunaID
                Ulasan: item.Ulasan || 'Permohonan Baru'
            }));
        } else {
            console.error('No data found for the provided MohonID.');
        }
    } catch (err) {
        console.error('Failed to fetch kronologi data:', err.response?.data || err.message);
    } finally {
        loading.value = false;
    }
};
...
onMounted(async () => {
    await namaUsers(); // Fetch user data first
    await getStatus();
    kronologi(MohonID); // Call the function on component mount
});

//options tindakan
const selectedTindakan = ref(''); // Holds the selected value

// function hantar keputusan

const hantarKeputusan = async () => {
    try {
        // Validation: Ensure all required fields are filled
        if (!selectedTindakan.value) {
            Swal.fire({
                icon: 'error',
                title: 'Ralat!',
                text: 'Sila pilih tindakan sebelum menghantar.'
            });
            return;
        }

        if (!selectedPeringkatKelulusan) {
            Swal.fire({
                icon: 'error',
                title: 'Ralat!',
                text: 'Sila pilih peringkat kelulusan sebelum menghantar.'
            });
            return;
        }

        const ulasanValue = document.getElementById('Ulasan').value.trim();
        const kewDiperakuValue = document.getElementById('KewDiperaku').value.trim();

        if (!ulasanValue) {
            Swal.fire({
                icon: 'error',
                title: 'Ralat!',
                text: 'Sila isi ulasan sebelum menghantar.'
            });
            return;
        }
        ...
        if (!kewDiperakuValue) {
            Swal.fire({
                icon: 'error',
                title: 'Ralat!',
                text: 'Sila isi implikasi kewangan sebelum menghantar.'
            });
            return;
        }

        const requestBody = {
            MohonID: MohonID, // from route param
            PindaanID: 1,
            TindakanID: parseInt(selectedTindakan.value, 10), // Convert to integer
            MohonStatusID: parseInt(selectedTindakan.value, 10) === 1 ? 3 : 9, // Set based on TindakanID
            Ulasan: ulasanValue,
            KewDiperaku: kewDiperakuValue,
            KuasaLulusID: selectedPeringkatKelulusan.value, // Ensure correct selection
            TkhSemak: new Date().toISOString(), // Full ISO-8601 format (YYYY-MM-DDTHH:mm:ss.sssZ)
            PenyemakID: 40
        };

        // Debugging logs before sending request
        console.log('selectedTindakan:', selectedTindakan.value);
        console.log('selectedPeringkatKelulusan:', selectedPeringkatKelulusan);
        console.log('requestBody:', JSON.stringify(requestBody));

        // Send request
        const response = await axios.post(`${baseUrl}/api/semakan`, requestBody);

        // Success alert
        Swal.fire({
            icon: 'success',
            title: 'Berjaya!',
            text: 'Keputusan telah dihantar.'
        }).then(() => {
            router.push({ name: 'semakanProsesKelulusan' });
        });

        console.log('Keputusan berjaya dihantar:', response.data);
    } catch (error) {
        console.error('Gagal menghantar keputusan:', error);

        // Error alert
        Swal.fire({
            icon: 'error',
            title: 'Ralat!',
            text: 'Gagal menghantar keputusan. Sila cuba lagi.'
        });
    }
};
</script>
...
<template>
    <div class="p-fluid formgrid grid">
        <div class="col-12">
            <div class="card">
                <h5 class="right-aligned-heading">Semak Permohonan ( Kod Mohon : {{ MohonID }} )</h5>
                    ...
                    <form>
                        <div class="borang-container">
                            <div class="borang-header">
                                <p class="borang-date" style="text-align: right; margin: 0">Tarikh: {{ formattedDate }}</p>
                            </div>
                            ...
                            <div class="borang-section">
                                ...
                                <div class="borang-row">
                                    <div class="borang-field">
                                        ...
                                        <span>{{ dropdownKategoriProgram.find((option) => option.KategoriProgramID === permohonan.KategoriProgramID)?.keterangan }}</span>
                                    </div>
                                    <div class="borang-field">
                                        <label>Kategori Peserta:</label>
                                        <span>{{ dropdownKategoriPeserta.find((option) => option.KatPesertaID === permohonan.KatPesertaID)?.Keterangan }}</span>
                                    </div>
                                    <div class="borang-field">
                                        <label>Kategori Perkhidmatan:</label>
                                        <span>{{ dropdownKategoriKhidmat.find((option) => option.JenisKhidmatID === permohonan.JenisKhidmatID)?.Keterangan }}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="borang-section">
                                ...
                                <div class="borang-row">
                                    <div class="borang-field">
                                        <label>Negara:</label>
                                        <span>{{ dropdownNegara.find((option) => option.NegaraID === permohonan.NegaraID)?.Keterangan }}</span>
                                    </div>
                                    <div class="borang-field">
                                        <label>Tempat:</label>
                                    ...
                                    <tbody>
                                        ...
                                        <tr v-for="(peserta, index) in pesertaList" :key="peserta.PenggunaID">
                                            ...
                                            <td class="border border-gray-300 px-4 py-2 text-center">
                                                <input type="radio" :checked="peserta.KetuaDelagasi === 1" :disabled="peserta.KetuaDelagasi !== 1" />
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
                                    ...
                                    <table class="table-auto w-full border-collapse border border-gray-300">
                                        ...
                                        <tbody>
                                            <tr v-for="(document, index) in documents" :key="document.ID">
                                                ...
                                                <td class="border border-gray-300 px-4 py-2 text-center">
                                                    <Button icon="pi pi-download" class="p-button-rounded p-button-info mr-1" title="Muat Turun" @click="downloadFile(document)" />
                                                </td>
                                            </tr>
                                        </tbody>
                                    </table>
                                ...
                                <div class="borang-section">
                                    ...
                                    <div class="borang-row">
                                        <div class="borang-field">
                                            ...
                                            <span>{{ dropdownBudgetOptions.find((option) => option.value === form.PengesahanRP)?.label }}</span>
                                        </div>
                                        <div class="borang-field">
                                        ...
                                        <table class="table-auto w-full border-collapse border border-gray-300">
                                            ...
                                            <tbody>
                                                <tr v-for="(row, index) in tableData" :key="index">
                                                    ...
                                                    <td class="border border-gray-300 px-4 py-2 text-center">{{ row.TkhHantar }}</td>
                                                    <td class="border border-gray-300 px-4 py-2 text-center">{{ row.NamaPegawai }}</td>
                                                    <td class="border border-gray-300 px-4 py-2 text-center">{{ row.Ulasan }}</td>
                                                </tr>
                                            </tbody>
                                        </table>
                                    </div>
                        ...
                        <div class="divider"></div>
                        <!-- Custom divider -->
                        <div class="form-section" style="display: grid; grid-template-columns: repeat(12, 1fr); gap: 1rem; align-items: flex-start">
                            <!-- Tindakan Heading -->
                            <h5 class="right-left-heading" style="grid-column: span 12">Tindakan</h5>

                            <!-- Radio Buttons -->
                            <div class="field-radiobutton" style="grid-column: span 3">
                                <RadioButton id="tindakan1" name="tindakan" value="1" v-model="selectedTindakan" />
                                <label for="tindakan1">Disemak</label>
                            </div>
                            <div class="field-radiobutton" style="grid-column: span 3">
                                <RadioButton id="tindakan2" name="tindakan" value="2" v-model="selectedTindakan" />
                                <label for="tindakan2">Kuiri</label>
                            </div>

                            <!-- Dropdown -->
                            <div class="field" style="grid-column: span 6">
                                <label for="dropdownPeringkatKelulusan">Peringkat Kelulusan</label>
                                <Dropdown id="peringkatKelulusan" v-model="selectedPeringkatKelulusan" :options="dropdownPeringkatKelulusan" optionLabel="keterangan" optionValue="PeringkatKelulusanID" placeholder="Pilih Peringkat" :required="true" />
                                <small v-if="isPeringkatRequiredError" class="p-error">Sila pilih Peringkat Kelulusan.</small>
                            </div>

                            <!-- Financial Implication -->
                            <div class="field" style="grid-column: span 6">
                                <label for="KewDiperaku">Implikasi Kewangan yang diperakukan (RM)</label>
                                <InputText id="KewDiperaku" required="true" style="width: 100%" type="number" />
                            </div>

                            <!-- Ulasan -->
                            <div class="field" style="grid-column: span 6">
                                <label for="Ulasan">Ulasan</label>
                                <Textarea id="Ulasan" required="true" rows="3" cols="20" style="width: 100%" />
                            </div>
                        </div>
                        <div style="width: 100%; display: flex; justify-content: center; align-items: center">
                            ...
                            />&nbsp;&nbsp;&nbsp;
                            <Button label="Hantar" icon="pi pi-send" class="p-button-sm p-button-success p-button-rounded" title="Hantar Keputusan" style="width: auto; font-size: 0.75rem; padding: 0.5rem 1rem" @click="hantarKeputusan" />
                        </div>
                    </form>
                </div>
            </div>
