...
const fetchRow4Data = async () => {
    try {
        const response = await axios.post(`${baseUrl}/api/peraku/getByMohonID`, {
            MohonID: MohonID,
        });
        const data = response.data[0]; 
        return {
            Bil: 3,
            Status: 'Perakuan',
            TarikhTindakan: data.TkhPeraku,
            NamaPegawai: userMap.value[data.PerakuID] || 'Tidak Dikenalpasti', // Map PenyokongID to Nama
            Ulasan: data.Ulasan,
        };
    } catch (error) {
        console.error('Error fetching row 4 data:', error);
        return null;
    }
};
...
onMounted(async () => {
    ...
    const row4Data = await fetchRow4Data();

    // Only add rows that are not null
    tableData.value = [row1Data, row2Data, row3Data, row4Data].filter(row => row !== null);
...
<template>
    <div class="p-fluid formgrid grid">
        <div class="col-12">
            <div class="card">
                <h5 class="right-aligned-heading">Lulus Permohonan ( Kod Mohon : {{ MohonID }} )</h5>
                    ...
                    <form>
                        ...
                        <div class="form-section"
                            ...
                            <div class="field-radiobutton" style="grid-column: span 3;">
                                <RadioButton id="category1" name="Diluluskan" value="Diluluskan" v-model="selectedTindakan"/>
                                <label for="category1">Diluluskan</label>
                            </div>
                            <div class="field-radiobutton" style="grid-column: span 3;">
                                <RadioButton id="category2" name="Tidak Diluluskan" value="Tidak Diluluskan" v-model="selectedTindakan"/>
                                <label for="category2">Tidak Diluluskan</label>
                            </div>
                            <div class="field-radiobutton" style="grid-column: span 3;">
                            ...
                            <div class="field" style="grid-column: span 12;">
                                ...
                                <Textarea id="description" required="true" rows="3" cols="20" style="width: 100%;" />
                            </div>
