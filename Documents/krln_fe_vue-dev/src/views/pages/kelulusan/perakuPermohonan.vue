...
const fetchRow3Data = async () => {
    try {
        const response = await axios.post(`${baseUrl}/api/sokong/getByMohonID`, {
            MohonID: MohonID,
        });
        const data = response.data[0]; 
        return {
            Bil: 3,
            Status: 'Sokongan',
            TarikhTindakan: data.TkhSokong,
            NamaPegawai: userMap.value[data.PenyokongID] || 'Tidak Dikenalpasti', // Map PenyokongID to Nama
            Ulasan: data.Ulasan,
        };
    } catch (error) {
        console.error('Error fetching row 3 data:', error);
        return null;
    }
};
...
onMounted(async () => {
    ...
    const row3Data = await fetchRow3Data();

    // Only add rows that are not null
    tableData.value = [row1Data, row2Data, row3Data].filter(row => row !== null);
...
// options Ulasan
const selectedOption = ref(""); // To track which option is selected
const selectedDropdown = ref(null); // Stores selected dropdown value
const ulasanText = ref(""); // Stores textarea value

// Dropdown options
const ulasanOptions = ref([
    { label: "Diperaku untuk persetujuan YBhg KSU untuk diangkat kepada MOF bagi kelulusan selaras dengan PB3.1", value: "Diperaku untuk persetujuan YBhg KSU untuk diangkat kepada MOF bagi kelulusan selaras dengan PB3.1" },
    { label: "Diperaku untuk persetujuan YBhg KSU berkenaan pertukaran pegawai untuk menghadiri kursus", value: "Diperaku untuk persetujuan YBhg KSU berkenaan pertukaran pegawai untuk menghadiri kursus" },
    { label: "Diperaku untuk persetujuan YBhg KSU berkenaan pertukaran pegawai untuk menghadiri persidangan", value: "Diperaku untuk persetujuan YBhg KSU berkenaan pertukaran pegawai untuk menghadiri persidangan" },
    { label: "Diperaku untuk persetujuan YBhg KSU berkenaan pertukaran pegawai untuk menghadiri mesyuarat", value: "Diperaku untuk persetujuan YBhg KSU berkenaan pertukaran pegawai untuk menghadiri mesyuarat" },
    { label: "Diperaku untuk kelulusan YBhg KSU", value: "Diperaku untuk kelulusan YBhg KSU" },
    { label: "Diperaku untuk ditolak YBhg KSU", value: "Diperaku untuk ditolak YBhg KSU" }
]);
                    ...
                    <form>
                        ...
                        <div class="form-section"
                            ...
                            <div class="field-radiobutton" style="grid-column: span 3;">
                                <RadioButton id="category1" name="Diperaku" value="Diperaku" v-model="selectedTindakan"/>
                                <label for="category1">Diperaku</label>
                            </div>
                            <div class="field-radiobutton" style="grid-column: span 3;">
                                <RadioButton id="category2" name="Tidak Diperaku" value="Tidak Diperaku" v-model="selectedTindakan"/>
                                <label for="category2">Tidak Diperaku</label>
                            </div>
                            <div class="field-radiobutton" style="grid-column: span 3;">
                                <RadioButton id="category2" name="Kuiri" value="Kuiri" v-model="selectedTindakan"/>
                                <label for="category2">Kuiri</label>
                            </div>
